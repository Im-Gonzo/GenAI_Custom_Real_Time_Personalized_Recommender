...
class QueryTowerFactory:
    ...
    def build(
        self, embed_sim: int = settings.TWO_TOWER_MODEL_EMBEDDING_SIZE
    ) -> "QueryTower":
        return QueryTower(
            user_ids=self._dataset.properties["user_ids"], emb_dim=embed_sim
        )
...
class ItemTower(tf.keras.Model):
    def __init__(
        self, item_ids: list, garment_groups: list, index_groups: list, emb_dim: int
    ):
        super().__init__()
