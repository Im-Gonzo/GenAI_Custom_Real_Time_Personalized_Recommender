    ...
    for col in df.columns:
        ...
        if missing_ratio > 0:
            ...
            if missing_ratio > threshold:
                logger.warning(f"Column {col} has {missing_ratio:.2%} missing values, exceeding threshold of {threshold:.2%}")
        ...
        for stat_name, expected_value in stats.items():
            ...
            if actual_value is not None:
                ...
                if isinstance(expected_value, (int, float)) and isinstance(actual_value, (int, float)):
                    rel_diff = abs(expected_value - actual_value) / max(abs(expected_value), 1e-10)
...
def check_data_freshness(
    ...
    if age_days > max_age_days:
        logger.warning(f"Data is {age_days} days old, exceeding maximum age of {max_age_days} days")
        return False
