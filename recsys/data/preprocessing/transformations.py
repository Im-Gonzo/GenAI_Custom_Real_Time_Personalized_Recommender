...
def normalize_numeric_features(
    df: pl.DataFrame, columns: Optional[List[str]] = None, method: str = "minmax"
) -> pl.DataFrame:
    """
    ...
    for col in columns:
        ...
        if method == "minmax":
            ...
            if max_val > min_val:
                ...
                result = result.with_columns([pl.lit(0.5).alias(f"{col}_norm")])
            ...
            if std_val > 0:
                ...
                result = result.with_columns([pl.lit(0.0).alias(f"{col}_norm")])
...
def encode_categorical_features(
    df: pl.DataFrame, columns: Optional[List[str]] = None, method: str = "one_hot"
) -> pl.DataFrame:
    """
