...
def train_test_split(
    ...
    if train_start is not None:
        ...
        train_df = df.filter(
            (pl.col(time_column) >= train_start) & 
            (pl.col(time_column) < train_end)
        )
        
        test_df = df.filter(
            (pl.col(time_column) >= test_start) & 
            (pl.col(time_column) < test_end)
        )
        ...
        if stratify_column is not None:
            ...
            for stratum in strata:
                stratum_indices = df.filter(pl.col(stratify_column) == stratum).row_indices()
...
def train_validation_test_split(
    ...
    test_size: float = 0.1,
    time_ranges: Optional[Dict[str, Tuple[Union[str, datetime, date], Union[str, datetime, date]]]] = None,
    time_column: str = "t_dat",
    stratify_column: Optional[str] = None,
    seed: int = 42,
) -> Tuple[pl.DataFrame, pl.DataFrame, pl.DataFrame, Optional[pl.DataFrame], Optional[pl.DataFrame], Optional[pl.DataFrame]]:
    """
    ...
    if time_ranges is not None:
        ...
        if stratify_column is not None:
            ...
            train_df = df_with_random.filter(pl.col("_random") <= val_threshold).drop("_random")
            val_df = df_with_random.filter(
                (pl.col("_random") > val_threshold) & (pl.col("_random") <= test_threshold)
            ).drop("_random")
            test_df = df_with_random.filter(pl.col("_random") > test_threshold).drop("_random")
