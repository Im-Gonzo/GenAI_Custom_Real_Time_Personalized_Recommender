...
def handle_missing_values(
    df: pl.DataFrame, strategy: Dict[str, str] = None
) -> pl.DataFrame:
    """
    ...
    for col, strat in strategy.items():
        ...
        if strat == "drop":
            ...
            mode_val = (
                df[col].value_counts().filter(pl.col(col).is_not_null()).head(1)[col][0]
            )
            ...
            result = result.with_columns([pl.col(col).fill_null(constant_val)])
